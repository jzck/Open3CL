...
export function conso_aux_gen(di, de, type, bch, bch_dep) {
  let type_generateur = enums[`type_generateur_${type}`][de[`enum_type_generateur_${type}_id`]]
  // find key in G that starts with type_generateur_ch
  let g = G[Object.keys(G).find((key) => type_generateur.startsWith(key))] || 0
  let h = H[Object.keys(G).find((key) => type_generateur.startsWith(key))] || 0
  let Paux_g_ch = g + h * (di.pn / 1000)
