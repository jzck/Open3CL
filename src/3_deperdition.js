...
function calc_Sdep(murs_list, porte_list, bv_list) {
	let Smurs_dep = murs_list.reduce((acc, mur) => {
	...
	let Sporte = porte_list.reduce(
		(acc, porte) => acc + Number(porte.donnee_entree.surface_porte),
		0
	);
	let Sbv = bv_list.reduce((acc, bv) => acc + Number(bv.donnee_entree.surface_totale_baie), 0);
	const Sdep = Smurs_dep + Sporte + Sbv;
	return Sdep;
}

export function Umur(o) {
	let de = o.donnee_entree;
	let di = o.donnee_intermediaire;
...
export default function calc_deperdition(cg, zc, th, ej, enveloppe, logement) {
	...
	let murs_list = mur_list.concat(ph_list);
	let Sdep = calc_Sdep(murs_list, porte_list, bv_list);

	vt_list.forEach((vt) => {
		calc_ventilation(vt, cg, th, Sdep, mur_list, ph_list, porte_list, bv_list);
	});

	let d_mur = mur_list.reduce((acc, mur) => acc + Umur(mur), 0);
	let d_pb = pb_list.reduce((acc, pb) => acc + Upb(pb), 0);
	let d_ph = ph_list.reduce((acc, ph) => acc + Uph(ph), 0);
	let d_bv = bv_list.reduce((acc, bv) => acc + Ubv(bv), 0);
	let d_porte = porte_list.reduce((acc, porte) => acc + Uporte(porte), 0);
	let d_pt = pt_list.reduce((acc, pt) => acc + Upt(pt), 0);
	let hvent = vt_list.reduce((acc, vt) => acc + vt.donnee_intermediaire.hvent, 0);
	let hperm = vt_list.reduce((acc, vt) => acc + vt.donnee_intermediaire.hperm, 0);
